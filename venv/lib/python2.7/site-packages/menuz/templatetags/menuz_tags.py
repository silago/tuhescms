...
def render_menu_children(request, parent, items, menu_tag):
    ...
    if childs:
        output.append('<%s class="dropdown">' % menu_tag)
        for menu in childs:
...
def list_menu(context, position_id):
    ...
    print get_menu_components(position_id)
    menu_tag, menu_class, before_link, after_link, ulclass = get_menu_components(position_id)
    ...
    for menu in items:
        if menu['parent_id'] == 0:
            ...
            if counter == 0:
                output.append('<li class="li_toplevel not-click first menu_%s %s">' % (menu['id'], current_class))
